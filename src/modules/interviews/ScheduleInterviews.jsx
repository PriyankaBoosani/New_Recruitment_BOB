  ...
  const uniqueAllocatedCentres = [
    ...new Map(
      sourceCandidates.map((candidate) => [
        ...
        {
          interviewCentreId: isEditMode ? candidate.interviewCenterId : candidate.interviewCenterId,

          interviewCentre: isEditMode ? candidate.zone : candidate.interviewCenterName,
        },
      ])
    ).values(),
  ];
  ...
  const rebuiltSelectedPanels = Object.values(
    (schedulePoolData || []).reduce((acc, item) => {
      ...
      configurations.forEach((config) => {
        ...
        const startTime = config?.startDatetime?.split("T")[1]?.slice(0, 5) || "";
        ...
        const existingSlot = acc[panelId].slots.find((slot) => slot.date === slotDate);
        ...
        if (existingSlot) {
          ...
          existingSlot.perDay = String(Math.floor(diffMins / Number(existingSlot.duration || 15)));
        }
    ...
    <div className="container-fluid px-4 py-3 mb-5 pb-5">
      ...
      <HeaderWithBack
        ...
        subtitle={`Scheduling for ${
          isEditMode ? schedulePoolData?.length || 0 : passedCandidates?.length || 0
        } candidates`}
        ...
        onBack={() => {
          navigate("/candidate-workflow", {
            state: {
              requisitionId: state.requisitionId,
              positionIds: Array.isArray(state.positionId) ? state.positionId : [state.positionId],
