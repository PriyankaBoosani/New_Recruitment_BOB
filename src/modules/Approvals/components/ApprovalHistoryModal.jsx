...
const ApprovalHistoryModal = ({ show, onClose, historyData = [], loading = false }) => {
  const historyArray = Array.isArray(historyData) ? historyData : [];
  const { t } = useTranslation("approvalHistory");

  return (
    <Modal show={show} onHide={onClose} centered size="lg" className="approval-history-modal">
                  ...
                  [...historyArray]
                    ...
                    .map((item) => (
                      <tr key={item.approvalId}>
                        <td className="fw-normal fs-14 mb-0">{item.approverName || "-"}</td>

                        <td className="fw-normal fs-14 mb-0">{formatDateTime(item.actionDate)}</td>

                        <td className="fw-normal fs-14 mb-0">{formatStatusLabel(item.status)}</td>

                        <td className="fw-normal fs-14 mb-0">{item.comments || "-"}</td>
                      </tr>
                    ))
                ) : (
                  <tr>
