...
const useExtensionRequests = () => {
  ...
  const fetchThreadMessages = useCallback(async (threadId) => {
    ...
    try {
      ...
      const res = await committeeManagementService.getMessagesByThreadId(threadId);
  ...
  const fetchExtensionRequests = useCallback(
    async ({
      ...
      try {
        ...
        const res = await committeeManagementService.getExtensionApprovals(body, {
          page,
          size,
        });
  ...
  const fetchApprovalHistory = useCallback(async (conversationThreadId) => {
    ...
    try {
      ...
      const [historyRes, usersRes] = await Promise.all([
        committeeManagementService.getApprovalHistoryByThreadId(conversationThreadId),
      ...
      const mappedHistory = Array.isArray(historyData)
        ? historyData.map((item) => ({
            ...item,
            approverName: userMap[item.approverId] || item.approverName || "Unknown User",
          }))
        : [];
      ...
      toast.error(error?.response?.data?.message || "Failed to load approval history");
      setHistoryData([]);
      return [];
    } finally {
