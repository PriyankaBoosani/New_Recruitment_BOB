...
const CommitteeRequests = () => {
  ...
  const handleApprovalAction = async (modalComment) => {
    ...
    if (actionType === "approve") {
      success = await approvePanels(ids, commentText, selectedPosition?.positionId);
    } else {
      success = await rejectPanels(ids, commentText, selectedPosition?.positionId);
    }
  ...
  const paginatedPanels = filteredPanels.slice(page * pageSize, page * pageSize + pageSize);

  const allSelected =
    filteredPanels.length > 0 && filteredPanels.every((p) => selectedReqIds.has(p.positionPanelId));
        ...
        <Row className="bulk-actions align-items-center mt-3 mb-3">
          <Col xs={12} md={6} className="selectcheck">
            <Form.Check
              ...
              onChange={(e) => {
                if (e.target.checked) {
                  setSelectedReqIds(new Set(filteredPanels.map((p) => p.positionPanelId)));
                } else {
                  setSelectedReqIds(new Set());
                }
              }}
          ...
          <>
            {filteredPanels.length === 0 ? (
              ...
              paginatedPanels.map((panelItem) => {
                ...
                const members = panel.panelMembers.map((m) => m.panelMember.name);
                  ...
                  >
                    <Row className="align-items-center gx-2 d-flex">
                      ...
                      <Col md={2} className="data-col">
                        ...
                        <div className="field-value">{panel.committee?.committeeName}</div>
                      </Col>
