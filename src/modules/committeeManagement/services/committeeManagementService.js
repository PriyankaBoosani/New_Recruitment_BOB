...
const committeeManagementService = {
  ...
  getRequisitions: (name = "") => api.get("/recruiter/job-requisitions/get-requisitions"),
  ...
  assignPanelToPosition: (jobPositionId, payload) =>
    api.post(`/recruiter/position-panel/save-or-update/${jobPositionId}`, payload),
  ...
  downloadPanelTemplate: () =>
    apis.get("/interview-panels/download-panel-template", { responseType: "blob" }),
  ...
  downloadPositionAssignmentTemplate: () =>
    api.get("/recruiter/position-panel/download-assignment-template", { responseType: "blob" }),
  ...
  getRequisitionApprovalHistory: (panelId) =>
    api.get(`/recruiter/workflow-approval/get-panels-approval-history/${panelId}`),
  ...
  submitForL1L2Approval: (body) => api.post("/recruiter/messages/submit-for-l1-l2-approval", body),
