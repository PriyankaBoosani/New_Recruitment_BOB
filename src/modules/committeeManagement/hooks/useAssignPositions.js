...
export const useAssignPositions = (userId) => {
  ...
  const loadPositionData = async (positionId) => {
    ...
    try {
      ...
      const assignedRes = await committeeManagementService.getPanelsByPosition(selectedPosition);
      ...
      const mapAssigned = (list) =>
        list.map((p) => {
          ...
          return {
            ...
            // display text
            positionPanelStatus: rawStatus.toLowerCase().replace("_", " ").replace(/^l1/, "L1"),
          };
        });
  ...
  const isPanelChanged = (panel, originalPanel) => {
    ...
    const originalMembers = (originalPanel.members || []).map((m) => m.userId).sort();
    ...
    try {
      ...
      Object.entries(selectedCommittees).forEach(([committeeType, panels]) => {
        panels.forEach((panel, seqIndex) => {
          const originalPanel = originalCommittees?.[committeeType]?.find((p) => p.id === panel.id);
          ...
          const actionEnum = !panel.positionPanelId ? "ADD" : isChanged ? "MODIFY" : null;
      ...
      const res = await committeeManagementService.assignPanelToPosition(selectedPosition, payload);
      if (res?.success) {
