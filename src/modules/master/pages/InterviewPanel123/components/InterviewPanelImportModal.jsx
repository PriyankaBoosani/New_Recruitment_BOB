...
const InterviewPanelImportModal = ({
  ...
  const handleFileChange = (e) => {
    ...
    if (isExcel) {
      ...
      setError(t("interviewPanel:invalid_file"));
    }
  };

  const handleUpload = async () => {
    if (!selectedFile) {
      setError(t("interviewPanel:no_file_selected"));
      return;
    }

    const result = await bulkAddPanels(selectedFile);
    ...
    <div>
      <div className="import-area p-4 rounded" style={{ background: '#fceee9' }}>
        <div className="text-center mb-3">
          ...
          <h5 className="mb-2 uploadfile">
            {t("interviewPanel:upload_panels")}
          </h5>

          <p className="text-muted small">
            {t("interviewPanel:support_xlsx")}
          </p>
        </div>
        ...
        <div className="text-center mb-3">
          <label htmlFor="upload-xlsx">
            <Button variant="primary" as="span" className="btnupload" disabled={loading}>
              {selectedFile
                ? t("interviewPanel:reupload_xlsx")
      ...
      <div className="d-flex justify-content-end gap-2 modal-footer-custom">
        ...
        <Button variant="primary" onClick={handleUpload}>
          {loading ? t("interviewPanel:importing") : t("interviewPanel:import")}
        </Button>
      </div>
    </div>
  );
