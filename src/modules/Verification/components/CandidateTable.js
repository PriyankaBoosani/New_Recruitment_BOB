  ...
  const goToPreview = (c) => {
    navigate("/candidate-preview", {
      state: {
        ...
        positionId: position?.raw?.positionId || position?.positionId || position?.value || null,
  ...
  return (
    <div className="verification-table-wrapper">
                  ...
                  <tr key={c.id}>
                    ...
                    <td className="text-center">
                      <input
                        type="checkbox"
                        checked={c.absent}
                        disabled={c.status !== "Pending" && c.status !== "Zonal Absent"}
      ...
      <div className="d-block d-md-none">
        {!isSelectionDone || filteredCandidates.length === 0 ? (
          <div className="text-center py-4 text-muted">{t("verification:no_candidates_found")}</div>
        ) : (
          filteredCandidates.map((c) => {
            return (
