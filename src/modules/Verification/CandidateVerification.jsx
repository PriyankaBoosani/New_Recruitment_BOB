...
export default function CandidateVerification() {
  ...
  const isBackNavigationRef = useRef(sessionStorage.getItem("fromPreviewBack") === "true");
  ...
  const handleViewFile = async (candidateRaw) => {
    ...
    try {
      ...
      const res = await masterApiService.getAzureBlobSasUrl(candidateRaw.resumeUrl, "candidate");
  ...
  const [selectedDate, setSelectedDate] = useState(navSelectedDate || new Date());
  ...
  const loadCandidates = async (dateParam = selectedDate) => {
    try {
      const res = await CandidateVerificationService.getCandidatesByDate(formatApiDate(dateParam));
      ...
      if (
        ...
        if (location.state?.requisition) setSelectedRequisition(location.state.requisition);

        if (location.state?.position) setSelectedPosition(location.state.position);
  ...
  const baseFiltered = allCandidates.filter((c) => {
    ...
    const searchMatch =
      c.name?.toLowerCase().includes(searchText.toLowerCase()) || c.regNo?.includes(searchText);
  ...
  const stageCounts = Object.keys(STAGE_STATUS_MAP).reduce((acc, key) => {
    acc[key] = baseFiltered.filter((c) => c.status === STAGE_STATUS_MAP[key]).length;
    return acc;
  }, {});
  ...
  const toggleAbsent = (id) => {
    setAllCandidates((prev) => prev.map((c) => (c.id === id ? { ...c, absent: !c.absent } : c)));
  };

  const anyAbsentChanged = baseFiltered.some((c) => originalAbsentMap[c.id] !== c.absent);
  ...
  return (
    <div className="container-fluid px-4 py-3 candidate-verification-page">
      ...
      <div className="stage-filter-row d-flex align-items-center gap-3">
        <span className="fs-14 text-muted">{t("verification:filter_by_stage")}:</span>
