...
const JobPostingsList = () => {
      ...
      try {
        ...
        setMasterData({
          reservationCategories: (
            masterRes.data?.reservationCategories || []
          ).map((c) => ({
          ...
          disabilityCategories: (
            masterRes.data?.disabilityCategories || []
          ).map((c) => ({
            id: String(c.disabilityCategoryId),
            code: c.disabilityCode,
          })),

          employmentTypes: (masterRes.data?.employementTypes || []).map(
            (e) => ({
  ...
  const handlePublish = async (req) => {
    try {
      ...
      const response = await requisitionApiService.publishDraftRequisition(
        req.parentRequisitionId
      );
