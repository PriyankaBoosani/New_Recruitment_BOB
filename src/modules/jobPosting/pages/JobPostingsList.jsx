...
const JobPostingsList = () => {
  ...
  const handleConfirmDeletePosition = async () => {
    ...
    await deletePosition(selectedPosition.requisitionId, selectedPosition.positionId);
  ...
  const handleSubmitForApproval = (postingStatus) => {
    ...
    const selectedVisibleRequisitions = visibleRequisitions.filter((r) => selectedReqIds.has(r.id));
    ...
    const ids = selectedVisibleRequisitions.filter((r) => r.status !== "Approved").map((r) => r.id);
    ...
    <Container fluid className="job-postings-page">
      ...
      <Row className="mb-3 align-items-center">
        ...
        <Col className="text-end create">
          <Button onClick={() => navigate("/job-posting/create-requisition")}>
            <img src={CreatePlus_Icon} alt="Create New Requisition" className="icon-16" />{" "}
      ...
      <Row className="filters-row g-2 mb-3">
        <Col xs={12} md={2}>
          ...
          <Form.Select value={year} className="yearfon" onChange={(e) => setYear(e.target.value)}>
            {yearOptions.map((yr) => (
              <option key={yr} value={yr}>
                {t("jobPostingsList:year_label")} - {yr}
              </option>
