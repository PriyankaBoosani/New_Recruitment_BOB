...
const ReservationSection = ({
  ...
  const groupedExServicemen = Object.values(
    ...
    ...group,
    label: group.items.map((item) => item.code).join("/"),
  }));
  return (
    <>
        ...
        <Col xs={12} className="mt-4">
            ...
            {formData.enableStateDistribution && (
              <div>
                <Form.Check
                  ...
                  onChange={(e) => {
                    if (isControlledEdit && !isProficientInLocalLanguage)
                      return;
          ...
          {!formData.enableStateDistribution ? (
            <Row className="g-4">
              ...
              <Col md={2}>
                <Card className="p-3 genfonts h-100">
                  <h6 className="text-primary mb-3">{t("addPosition:ex_servicemen")}</h6>

                  <Row className="g-3">
                    {groupedExServicemen.map((group) => (
                      <Col md={6} key={group.groupId}>
                        <Form.Label className="small fw-semibold">
                          {group.label}
                        </Form.Label>
                        ...
                        <Form.Control
                          ...
                          inputMode="numeric"
                          pattern="[0-9]*"
                          value={nationalExServicemen?.[group.groupId] ?? "0"}
                          onChange={(e) => {
                            ...
                            setNationalExServicemen((prev) => ({
                              ...prev,
                              [group.groupId]: value === "" ? "0" : value,
                            }));
            ...
            <>
              <Row className="g-3 mb-3">
                ...
                <Col md={3}>
                  ...
                  <ErrorMessage>
                    {renderError(errors.stateLanguage)}
                  </ErrorMessage>
                </Col>
              </Row>
                      ...
                      {disabilityCategories.map((d) => (
                        <Col md={3} key={d.id}>
                          ...
                          <Form.Control
                            ...
                            pattern="[0-9]*"
                            value={
                              currentState.disabilities?.[d.disabilityCode] ??
                              "0"
                            }
                            onChange={(e) => {
                              let value = e.target.value;
                      ...
                      {groupedExServicemen.map((group) => (
                        <Col md={6} key={group.groupId}>
                          ...
                          <Form.Control
                            ...
                            pattern="[0-9]*"
                            value={
                              currentState.exServicemen?.[group.groupId] ?? "0"
                            }
                            onChange={(e) => {
                              let value = e.target.value.replace(/\D/g, "");
              ...
              <ErrorMessage>
                {renderError(errors.stateDistribution)}
              </ErrorMessage>
                ...
                <table className="table table-bordered">
                  <thead>
                    ...
                    <tr>
                      ...
                      <th
                        colSpan={groupedExServicemen.length}
                        className="text-center bgcol"
                      >
                        {t("addPosition:ex_servicemen")}
                      </th>
                    ...
                    <tr>
                      ...
                      {groupedExServicemen.map((group) => (
                        <th key={group.groupId}>{group.label}</th>
                      ))}
                          ...
                          <tr key={idx}>
                            ...
                            {groupedExServicemen.map((group) => (
                              <td key={group.groupId}>
                                {row.exServicemen?.[group.groupId] ?? 0}
                              </td>
                            ))}
          ...
          <ErrorMessage>
            {renderError(errors.nationalDistribution)}
          </ErrorMessage>
        </Col>
      </fieldset>
    </>
