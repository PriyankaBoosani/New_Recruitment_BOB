  ...
  return (
    <Modal show={show} onHide={onClose} centered backdrop="static" className="modalimport">
      ...
      <Modal.Body className="text-center">
        {icon && <img src={icon} alt="icon" className="mb-3" style={{ width: 40 }} />}

        <p className="mb-1 f14">{message || t("importModal:confirm_default_message")}</p>

        {itemLabel && <strong className="d-block">{itemLabel}</strong>}

        {showWarning && <p className="text-muted mt-2">{t("importModal:cannot_undo")}</p>}
      </Modal.Body>
      ...
      <Modal.Footer className="mt-3">
        ...
        <Button variant={confirmVariant} onClick={onConfirm} disabled={loading} className="f14">
          {loading ? t("importModal:processing") : confirmText || t("importModal:confirm_button")}
        </Button>
      </Modal.Footer>
    </Modal>
  );
