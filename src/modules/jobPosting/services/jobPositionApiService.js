...
const jobPositionApiService = {
  getPositionsByRequisition: (requisitionId) =>
    api.get(`/recruiter/job-positions/get-job-position-by-requisition/${requisitionId}`),
  ...
  deletePositionById: (positionId) =>
    api.delete(`/recruiter/job-positions/delete-job-position-by-id/${positionId}`),

  getRequisitionById: (id) => api.get(`/recruiter/job-requisitions/${id}`),

  downloadTemplate: () =>
    api.get("/recruiter/job-positions/download-template", { responseType: "blob" }),

  bulkImport: (requisitionId, file) => {
    const formData = new FormData();
    formData.append("file", file);

    return api.post(`/recruiter/job-positions/create-bulk-positions/${requisitionId}`, formData, {
      headers: {
        "Content-Type": "multipart/form-data",
        "X-Client": "AzureAD",
      },
  ...
  getScreeningCommitteeStatus: (applicationId) =>
    api.get(`/recruiter/document-verification/get-screening-committee/${applicationId}`, {
      headers: {
        "X-Client": "AzureAD",
      },
    }),
  ...
  saveScreeningDecision: (payload) =>
    api.post("/recruiter/document-verification/save-screening-committee/verify", payload, {
      headers: { "X-Client": "AzureAD" },
    }),

  saveCandidateDiscrepancyDetails(payload) {
    return api.post("/recruiter/candidate-screening/save-candidate-discrepancy-details", payload, {
      headers: {
        "X-Client": "AzureAD",
      },
    });
  ...
  submitOverallZonalVerification(payload) {
    return api.post("/recruiter/zonal-verification/submit-overall-verification", payload);
  },
  ...
  updateZonalAbsent(applicationId, isAbsent) {
    return api.post(`/recruiter/zonal-verification/update-absent-status`, null, {
  ...
  getCandidateDiscrepancyDetails(applicationId) {
    return api.get("/recruiter/candidate-screening/get-candidate-discrepancy-details", {
  ...
  getRequisitionApprovalHistory: (requisitionId) =>
    api.get(`/recruiter/workflow-approval/get-requisition-approval-history/${requisitionId}`, {
      headers: {
        "X-Client": "AzureAD",
      },
    }),
  ...
  sendToOfferPool(applicationIds) {
    return api.post("/recruiter/candidate-offer/send-to-offer-pool", applicationIds, {
      headers: {
        "X-Client": "AzureAD",
      },
    });
  ...
  downloadRankListExcel: (offerIds) => {
    return api.post("/recruiter/candidate-offer/download-offers-excel", offerIds, {
      headers: {
        "X-Client": "AzureAD",
      },
  ...
  uploadRanksExcel: (file) => {
    ...
    return formDataApi.post("/recruiter/candidate-offer/upload-ranks-excel", formData, {
      headers: {
        "X-Client": "AzureAD",
      },
    });
