...
const validateDistribution = ({
  ...
  if (formData.enableStateDistribution) {
    ...
    const exServicemenTotal = Object.values(nationalExServicemen || {}).reduce(
      (sum, v) => sum + Number(v || 0),
      0
    );
    ...
    } else if (exServicemenTotal > categoryTotal) {
      errors.nationalDistribution = {
        key: "validation:exservicemen_exceeds_category",
        params: {
          exServicemenTotal,
          categoryTotal,
        },
...
export const validateStateDistribution = ({
  ...
  const exTotal = Object.values(currentState.exServicemen || {}).reduce(
    (a, b) => a + Number(b || 0),
    0
  );
  ...
  } else if (exTotal > catTotal) {
    errors.stateDistribution = {
      key: "validation:exservicemen_exceeds_category",
      params: {
        exServicemenTotal: exTotal,
