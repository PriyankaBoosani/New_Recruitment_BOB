...
export default function InterviewerSchedule() {
  ...
  const cameFromPreviewBack = sessionStorage.getItem("fromPreviewBack") === "true";
  ...
  const navReqId = navState.requisition?.requisition?.id || navState.requisition?.id || null;

  const navPosId = navState.position?.position?.positionId || navState.position?.positionId || null;

  useEffect(() => {
    if (sessionStorage.getItem("fromPreviewBack") === "true" && navState.selectedDate) {
  ...
  useEffect(() => {
    //  ADD THIS BLOCK
    if (cameFromPreviewBack && navState.preloadedCandidates?.length && selectedPosition) {
      return;
    }
    ...
    const load = async () => {
      try {
        ...
        const res = await InterviewerService.getCandidatesByPositionAndDate(posId, dateStr);
  ...
  useEffect(() => {
    ...
    const normalizedReq = {
      ...
      startDate: navState.requisition.startDate ?? navState.requisition.registration_start_date,
      endDate: navState.requisition.endDate ?? navState.requisition.registration_end_date,
    };
  ...
  const filteredRows = useMemo(() => {
    ...
    return rows.filter(
      (r) => r.name.toLowerCase().includes(text) || r.regNo.toLowerCase().includes(text)
    );
  }, [rows, searchText]);
  ...
  const updateComment = (id, val) =>
    setRows((prev) => prev.map((r) => (r.id === id ? { ...r, comment: val } : r)));

  const updateScore = (id, val) =>
    setRows((prev) => prev.map((r) => (r.id === id ? { ...r, score: val } : r)));
  ...
  const handleViewFile = async (raw) => {
    ...
    try {
      setLoadingPdf(true);
      const sas = await masterApiService.getAzureBlobSasUrl(raw.resumeUrl, "candidate");
      setPdfUrl(sas?.trim());
    ...
    <div className="container-fluid px-4 py-3 candidate-verification-page">
      ...
      <div className="verification-toolbar">
        <div className="date-nav">
          <span className="nav-arrow" onClick={() => setSelectedDate((d) => subDays(d, 1))}>
            ‹
          </span>
            ...
            onSuccess={async () => {
              ...
              try {
                ...
                if (selectedPosition?.position?.positionId) {
                  ...
                  const candRes = await InterviewerService.getCandidatesByPositionAndDate(
