        ...
        <table className="table table-bordered small text-center mb-0">
          <thead>
            <tr>
              ...
              <th className="light_font fw-600 fs-13" colSpan={reservationCategories.length + 1}>
                {t("category")}
              </th>
              <th className="light_font fw-600 fs-13" colSpan={disabilityCategories.length}>
                {t("disability")}
              </th>
            </tr>
            <tr>
              {reservationCategories.map((cat) => (
                <th className="light_font fw-600 fs-12" key={cat.reservationCategoriesId}>
                  {cat.categoryCode}
                </th>
              ))}
              ...
              {disabilityCategories.map((d) => (
                <th className="light_font fw-600 fs-12" key={d.disabilityCategoryId}>
                  {d.disabilityCode}
                </th>
              ))}
            </tr>
            ...
            {positionStateDistributions.map((state, idx) => (
              <tr key={idx}>
                <td className="fw-400" style={{ padding: "12px 6px", textAlign: "left" }}>
                  {stateMap[String(state.stateId || state.zonalStateID).toLowerCase()] ||
                    t("unknown")}
                </td>
                <td className="fw-400" style={{ padding: "12px 6px", textAlign: "left" }}>
