...
export default function CompensationPool({
  ...
  const canEditCompensation =
    selectedCandidate?.status === "SUBMITTED" || selectedCandidate?.status === "RENEGOTIATE";

  const canEditManagerCompensation =
    selectedCandidate?.status !== "NEW" && selectedCandidate?.status !== "APPROVED"; //  ADD THIS
  ...
  const isCTCMatching = Number(fixedPay || 0) + Number(variablePay || 0) === Number(expectedCTC);
  ...
  const handleManagerAction = async (actionType) => {
    try {
      const fixed = parseAmount(managerForm.fixedPay) || selectedCandidate.fixedPay || 0;

      const variable = parseAmount(managerForm.variablePay) || selectedCandidate.variablePay || 0;
      ...
      const payload = {
        compensation: {
          ...
          fixedPay: parseAmount(managerForm.fixedPay) || selectedCandidate.fixedPay || 0,

          variablePay: parseAmount(managerForm.variablePay) || selectedCandidate.variablePay || 0,

          joiningBonus:
            parseAmount(managerForm.joiningBonus) || selectedCandidate.joiningBonus || 0,
          agreedCtc:
            (parseAmount(managerForm.fixedPay) || 0) + (parseAmount(managerForm.variablePay) || 0),

          hike: selectedCandidate.currentCtc
    ...
    try {
      ...
      const payload = {
        compensation: {
          ...
          agreedCtc:
            (parseAmount(formData.fixedPay) || 0) + (parseAmount(formData.variablePay) || 0),
      ...
      //  SUCCESS CASE
      if (responseData?.success === true) {
        toast.success(responseData?.message || "Compensation saved successfully");
        ...
        //  ERROR CASE
        const errorMsg = responseData?.data || responseData?.message || "Something went wrong";
    ...
    <>
        ...
        <table className="table table-hover mb-0">
          ...
          <thead className="bg-light">
            <tr>
              ...
              <th className="fs-14 fw-normal py-3" onClick={() => requestSort("name")}>
                Candidate {sortIcon("name")}
              </th>

              <th className="fs-14 fw-normal py-3" onClick={() => requestSort("currentCtc")}>
                Current CTC {sortIcon("currentCtc")}
              </th>

              <th className="fs-14 fw-normal py-3" onClick={() => requestSort("expectedCtc")}>
            ...
            {sortedCandidates.length === 0 ? (
              ...
              sortedCandidates.map((c) => (
                <tr key={c.id}>
                  ...
                  <td className="fs-14 align-content-center">{c.currentCtc || "-"}</td>
                  <td className="fs-14 align-content-center">{c.expectedCtc || "-"}</td>
                  ...
                  <td className="fs-14 align-content-center">{c.agreedCtc || "-"}</td>

                  <td className="fs-14 align-content-center">
                    {isRecruiter ? c.recruiterComments || "-" : c.panelComments || "-"}
                  </td>
                  ...
                  <td className="text-center align-content-center">
                    {/* Profile */}
                    <OverlayTrigger placement="bottom" overlay={<Tooltip>View Profile</Tooltip>}>
                      <Person
                    ...
                    {/* Document */}
                    <OverlayTrigger placement="bottom" overlay={<Tooltip>View Resume</Tooltip>}>
                      <FileText
                        className="me-3 cursor-pointer"
                        onClick={() => {
                          if (onViewFile) {
      ...
      >
        <Modal.Header closeButton className="custom-modal-header border-0">
          <Modal.Title className="fw-semibold fs-5">Agreed Compensation</Modal.Title>
        </Modal.Header>
    ...
    <div className="invalid-feedback">Fixed Pay is required</div>
            ...
            <div className="col-md-12">
              ...
              <textarea
                ...
                onChange={(e) => setFormData({ ...formData, recruiterComments: e.target.value })}
              />
            </div>
          </div>
        </Modal.Body>
        ...
        <Modal.Footer className="border-0 pt-2">
          <Button variant="light" className="px-4" onClick={() => setShowRecruiterModal(false)}>
            Cancel
          </Button>
    ...
    <div className="invalid-feedback">Fixed Pay is required</div>
            ...
            <div className="col-md-12">
              ...
              <textarea
                ...
                onChange={(e) => setManagerForm({ ...managerForm, panelComments: e.target.value })}
              />
            </div>
          </div>
        </Modal.Body>
