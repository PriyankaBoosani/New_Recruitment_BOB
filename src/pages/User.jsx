...
import '../css/user.css';
import { validateUserForm } from '../validators/user-validations';
...
const User = () => {
    ...
    const [deleteTarget, setDeleteTarget] = useState(null); // { id, name, email } or null
    ...
    // CSV / XLSX and import logic unchanged (kept from your original)
    const downloadTemplate = (type) => {
        const headers = ['role', 'name', 'email', 'mobile', 'password'];
        const sample = ['Manager', 'Sample User', 'sample@example.com', '9876500000', 'Password123'];
    ...
    const parseCSVTextToUsers = (text) => {
    ...
    const handleImport = async () => {
        ...
        if (selectedCSVFile) {
            try {
                ...
                const parsed = parseCSVTextToUsers(text);

                const nextIdStart = Math.max(0, ...users.map(u => u.id)) + 1;
                const newUsers = parsed.map((p, i) => ({
                    id: nextIdStart + i,
                    role: p.role,
                    name: p.name,
                    email: p.email,
                ...
                if (newUsers.length === 0) {
                    ...
                    alert(`Imported ${newUsers.length} users from CSV.`);
    ...
    const filteredUsers = users.filter(user =>
        Object.values(user).some(v => v.toString().toLowerCase().includes(searchTerm.toLowerCase()))
    );
                                    ...
                                    <tr key={user.id}>
                                        ...
                                        <td>
                                            <div className="action-buttons">
                                                ...
                                                >
                                                    <img src={deleteIcon} alt="Edit" className='icon-16' />
                                                </Button>
                                            </div>
                                        </td>
                                    </tr>
