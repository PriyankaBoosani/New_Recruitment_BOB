...
export const validateLanguages = (list = []) => {
  if (!list || list.length === 0) {
    return i18n.t("stateLanguages:languages_required", "Please select at least one language");
  }
  const seen = new Set();
  for (let val of list) {
    if (seen.has(val)) {
...
export const validateStateLanguageForm = (formData = {}, options = {}) => {
  ...
  if (!stateError) {
    ...
    if (isDuplicate) {
      errors.state = i18n.t("stateLanguages:duplicate_state", "State already exists");
    }
  }
  return {
