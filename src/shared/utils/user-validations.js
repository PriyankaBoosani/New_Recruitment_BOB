...
import { requiredField, minLength, maxLength, emailFormat } from "./common-validations";
...
export const validatePasswordConfirmation = (confirmPassword, password) => {
  ...
  if (confirmPassword !== password) return i18n.t("validation:passwords_not_match");
  ...
  if (!skipEmailCheck) {
    ...
    if (emailError) {
      ...
      const duplicateEmail = existing.find(
        (user) =>
          user.email && normalizeString(user.email) === emailNorm && user.userId !== currentId
      );
