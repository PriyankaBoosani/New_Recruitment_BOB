...
export const validateJobGradeForm = (formData = {}, options = {}) => {
  ...
  if (!Number.isNaN(minNum) && !Number.isNaN(maxNum) && maxNum > 0 && minNum > maxNum) {
    errors.minSalary = errors.minSalary || i18n.t("jobGrade:min_less_than_max");

    errors.maxSalary = errors.maxSalary || i18n.t("jobGrade:max_greater_than_min");
  }
  ...
  if (!errors.scale) {
    ...
    const duplicate = existing.find(
      (e) => normalize(e.scale) === scaleNorm && (currentId == null || e.id !== currentId)
    );
    if (duplicate) {
  ...
  if (!errors.gradeCode) {
    ...
    const duplicate = existing.find(
      (e) => normalize(e.gradeCode) === codeNorm && (currentId == null || e.id !== currentId)
    );
    if (duplicate) {
